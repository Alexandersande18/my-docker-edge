...
func (msg *Message) GetContent() map[string]interface{} {
	return msg.Content.(map[string]interface{})
}

func (msg *Message) GetContentRaw() interface{} {
	return msg.Content
}
...
func NewRegisterMessage(source string, ip string) *Message {
	msg := NewMessage("")
	msg.SetRoute(source, "0", "Master")
	msg.SetResourceOperation(ResourceTypeNode, RegisterOperation)
	msg.FillBody(NodeRegisterInfo{LocalIP: ip})
	return msg
}

func (msg *Message) SetSync() {
...
func (msg *Message) Clone(message *Message) *Message {
	...
	return NewRawMessage().BuildHeader(msgID, message.GetParentID(), message.GetTimestamp()).
		BuildRouter(message.GetSource(), message.GetGroup(), message.GetTarget(), message.GetResource(), message.GetOperation()).
...
func NewClientOfflineErrMessage(parentID string) *Message {
	return NewMessage(parentID).SetResourceOperation(ResourceTypeNull, ResponseErrorOperation)
}
